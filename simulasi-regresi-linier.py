...
# R2
axR2 = plt.axes([2*a + w, h-0.45, w/2, 0.08])
axR2.axis('off')
axR2.text(x=0, y=0, s="R2: ")
txR2 = axR2.text(x=0.25, y=0, s="0")

# MSE
axMSE = plt.axes([2*a + w, h-0.55, w/2, 0.08])
axMSE.axis('off')
...
def new_random_data(e):
    ...
    Y = -2*np.random.rand(10)
    Y = np.round(Y + 2*X+1, 2)
    [x_, y_, b0, b1, err_, R2, MSE] = reg_lin(X, Y)
    sldB0.set_val(b0)
    sldB1.set_val(b1)
    drawchart(X, Y, x_, y_, b0, b1, err_, R2, MSE)
...
def reg_lin(x, y, b0=None, b1=None):
    ...
    return(xa, ya, b0, b1, err_, R2, MSE)


def drawchart(x, y, x_, y_, b0, b1, err_, R2, MSE):
    axChart.clear()
    axChart.set_ylim([ylim0, ylim1])
    axChart.plot(x, y, '.r')
    axChart.plot(x_, y_, 'b')
...
axB0 = fig.add_axes( [2*a + w, h-0.2, w/2, 0.08] )
sldB0 = Slider(ax=axB0, label="B0", valmin=b0-2, valmax=b0+2, valinit=b0)

axB1 = fig.add_axes( [2*a + w, h-0.3, w/2, 0.08] )
...
def setB0(e):
    b0 = sldB0.val
    (x_, y_, b0, b1, err_, R2, MSE)=reg_lin(x=X, y=Y, b0=b0)
    drawchart(X, Y, x_, y_, b0, b1, err_, R2, MSE)


def setB1(e):
    b1 = sldB1.val
    (x_, y_, b0, b1, err_, R2, MSE)=reg_lin(x=X, y=Y, b1=b1)
    drawchart(X, Y, x_, y_, b0, b1, err_, R2, MSE)
